...
if $cygwin ; then
    ...
    for arg in "$@" ; do
        ...
        if [ $CHECK -ne 0 -a $CHECK2 -eq 0 ] ; then
            eval `echo "arg=$arg" | cygpath --path --ignore --file -`
        fi
        NEWARGSTR="$NEWARGSTR \"$arg\""
    done
    set -- $NEWARGSTR
fi

# Collect all arguments for the java command, preserving white space
collect_args () {
    for arg do
        printf %s\\n "$arg" | sed "s/'/'\\\\''/g;1s/^/'/;\$s/\$/'/"
    done
